        ...
        >
            ...
            <View style={[styles.card, Shadow.card]}>
              ...
              <View style={styles.progressTrack}>
                <View style={[styles.progressFill, {
                  ...
                  backgroundColor: completionPct >= 90 ? COLORS.success : completionPct >= 76 ? COLORS.orange : COLORS.error,
                }]} />
              </View>
          ...
          <View style={[styles.card, Shadow.card]}>
            ...
            <InfoRow
              ...
              label="Store"
              value={[dashboard?.user.store_name, profileUser?.store_name].find(Boolean) ?? '—'}
            />
            <InfoRow
              icon={<MapPin size={16} color="#C95F18" />}
              label="Store Code · Region"
              value={
                [dashboard?.user.store_code, dashboard?.user.region ?? profileUser?.region]
                  .filter(Boolean).join(' · ') || '—'
              }
            />
            <InfoRow
              icon={<MapPin size={16} color="#C95F18" />}
              label="City · State"
              value={[dashboard?.user.city, dashboard?.user.state].filter(Boolean).join(' · ') || '—'}
            />
          ...
          <View style={[styles.card, Shadow.card]}>
            ...
            <InfoRow icon={<IdCard size={16} color="#C95F18" />} label="Employee Code" value={dashboard?.user.employee_code ?? profileUser?.employee_code ?? '—'} />
            <InfoRow icon={<Phone size={16} color="#C95F18" />} label="Mobile" value={profileUser?.mobile ?? '—'} />
            <InfoRow icon={<Mail size={16} color="#C95F18" />} label="Email" value={dashboard?.user.email ?? profileUser?.email ?? '—'} />
            <InfoRow icon={<Building2 size={16} color="#C95F18" />} label="Department" value={dashboard?.user.department ?? profileUser?.department ?? '—'} />
            <InfoRow icon={<BadgeCheck size={16} color="#C95F18" />} label="Designation" value={dashboard?.user.designation ?? profileUser?.designation ?? '—'} />
            <InfoRow
          ...
          {profile ? (
            <View style={[styles.card, Shadow.card]}>
              ...
              <InfoRow
                ...
                value={profile.emergency_contact_name ? `${profile.emergency_contact_name} · ${profile.emergency_contact_mobile ?? ''}` : '—'}
              />
