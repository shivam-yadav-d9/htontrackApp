...
import type { AttendanceCorrection } from '@/types/approval.types';

export default function ManagerAttendanceCorrectionsScreen() {
  ...
  async function loadCorrections() {
    try {
      setLoading(true);
      const data = await approvalService.getManagerAttendanceCorrections();
      setCorrections(data);
    } catch {
      // silent
    } finally {
  ...
  async function review(item: AttendanceCorrection, status: 'approved' | 'rejected') {
    ...
    try {
      await approvalService.reviewAttendanceCorrection(item.id, {
        status,
        manager_remarks: remarks.trim() || undefined,
      });
      await loadCorrections();
      Alert.alert('Updated', `Correction request ${status}.`);
    } catch (err) {
      Alert.alert('Error', err instanceof Error ? err.message : 'Unable to review correction.');
    }
  }

  const pendingCount = corrections.filter((x) => x.status === 'pending').length;
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerSub}>Review staff attendance correction requests</Text>
      </View>
      ...
      <ScrollView
        ...
        <View style={styles.summaryCard}>
          <Clock3 size={22} color="#C95F18" />
          <Text style={styles.summaryText}>{pendingCount} pending correction{pendingCount !== 1 ? 's' : ''}</Text>
        </View>
              ...
              <View key={item.id} style={styles.requestCard}>
                <View style={styles.requestTop}>
                  <View style={{ flex: 1 }}>
                    <Text style={styles.requestTitle}>
                      {item.staff_name} · {item.correction_type.replace(/_/g, ' ')}
                    </Text>
                ...
                {(item.requested_check_in_time || item.requested_check_out_time) && (
                  <View style={styles.timeRow}>
                    {item.current_check_in_time && (
                      <Text style={styles.timeMeta}>Current In: {item.current_check_in_time}</Text>
                    )}
                    {item.requested_check_in_time && (
                      <Text style={styles.timeRequested}>→ In: {item.requested_check_in_time}</Text>
                    )}
                    {item.current_check_out_time && (
                      <Text style={styles.timeMeta}>Current Out: {item.current_check_out_time}</Text>
                    )}
                    {item.requested_check_out_time && (
                      <Text style={styles.timeRequested}>→ Out: {item.requested_check_out_time}</Text>
                    )}
                  </View>
                )}
...
const styles = StyleSheet.create({
  ...
  timeRow: { gap: 3 },
  timeMeta: { color: '#8A8178', fontSize: 11, fontWeight: '700' },
  timeRequested: { color: '#166534', fontSize: 11, fontWeight: '900' },
